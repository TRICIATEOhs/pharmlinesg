...
import productJson from "../../../data/products/wristbands/wristbands.js";
...
function Wristbands () {
    ...
    const header = "Wristbands";
    const category = " " + header;

    const productList = productJson && productJson.map((product) =>(
    ...
    const displayItemModal = (message) => {
        ...
        setImageGallery(message.imageGallery);
        setShow(true);
    }
    ...
    return (
        <>
            ...
            <div>
                {show ? (
                    <ItemModal handleCallBack={callBack} name={name} image={image} description={description} packSize={packSize} productOf={productOf} exclusiveAvailability={exclusiveAvailability} ingredients={ingredients} dosageInstructions={dosageInstructions} imageGallery={imageGallery}/>
                ) : (
                    ""
                )}
            </div>
